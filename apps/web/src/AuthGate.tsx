...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  const exchange = useCallback(async (code: string) => { if (!client) return; setState("signing_in"); setError(""); const { data, error: exchangeError } = await client.auth.exchangeCodeForSession(code); if (exchangeError || !data.user) { setError("Sign-in could not be completed."); setState("error"); return; } setUser(data.user); setState("signed_in"); }, [client]);
  ...
  const signOut = async () => { await client?.auth.signOut(); await window.docSync?.authStorage?.remove(); setUser(null); setState("signed_out"); };
