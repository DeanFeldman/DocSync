...
export class CloudSnapshotRepository {
  ...
  async getSnapshot(snapshotId: string) { const { data, error } = await this.client.from("workspace_snapshots").select("id,user_id,device_id,sha256,archive_size_bytes,workspace_revision,created_at").eq("id", validUuid(snapshotId)).maybeSingle(); if (error) throw classify(error, "provider_unavailable"); return data; }
