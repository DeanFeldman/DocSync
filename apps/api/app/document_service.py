...
def serialize_cached_word_preview(
    session: Session,
    document: DocumentRecord,
    version: DocumentVersion,
) -> dict:
    """Describe an existing immutable Word PDF without starting Word."""

    output_path = rendered_pdf_path(document, version.id)
    if not output_path.is_file() or output_path.stat().st_size == 0:
        raise HTTPException(
            status_code=409,
            detail="The Word preview is still being rendered.",
        )
    ...
    view.update(
        {
            ...
            "pagination": "word",
            "pdf_url": (
                f"/api/document-versions/{version.id}/rendered-file"
                f"?v={output_path.stat().st_mtime_ns}"
            ),
            "notice": (
                "Word layout preview generated by Microsoft Word. The PDF is a fixed "
                "snapshot; final wrapping and pagination are recalculated after generation."
            ),
            "render_map_status": render_map["status"],
    ...
    view.update(
        {
            ...
            "notice": (
                "Word layout preview generated by Microsoft Word. Selectable areas are "
                "prepared in the background; Select from structure remains available."
            ),
        }
    )
    if version is not None:
...
def generate_versions(
    ...
    try:
        ...
        # Word/PDF and coordinate caches are immutable-version artifacts. The
        # newly generated versions receive new IDs and therefore new render
        # keys; retaining the previous version cache avoids racing an active
        # background mapper and keeps historical previews valid.
